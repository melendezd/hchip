...
currentOp !m = readOp ((m^.cpu.memory) ! (1*pc)) ((m^.cpu.memory) ! (1*pc + 1))
    where pc = m^.cpu.registerFile.regPC
...
full n3 n2 n1 n0 = n0' + 0x10*n1' + 0x100*n2' + 0x1000 * n3'
    where n0' = fromIntegral n0 :: Word16
...
applyOp CLS m = m & over display (// zip allPoints (repeat 0))
                  & drawFlag .~ True
    where allPoints = range $ bounds (m^.display)

applyOp (DRW regXInd regYInd size) m = m & over display (updateInPlace xor spriteAssocsOnDisplay)
    ...
    where 
        ...
        !collision = or $ fmap spriteMatchesDisplay spriteIndices 
            where spriteMatchesDisplay pos = m^.display.ix (wrap . translate $ pos) .&. (sprite ! pos) == 1
...
applyOpCPU (LD regIndDest regIndSrc) cpu = cpu & over (registerFile.regV) (// [(regIndDest, regSrc)])
    where regSrc = (cpu^.registerFile.regV) ! regIndSrc
applyOpCPU (OR regIndDest regIndSrc) cpu = cpu & registerFile.regV.ix regIndDest .~ regDest .|. regSrc
    where regSrc  = cpu^.registerFile.regV.ix regIndSrc
...
applyOpCPU (SHL regIndDest regIndSrc) cpu = cpu 
...
applyOpCPU (RND regSrcInd val) cpu = cpu & regFile.regV.ix regSrcInd .~ (randVal .&. val)
                                        & seed .~ g
    where (randVal, g) = genWord8 (cpu^.seed)
...
updateBy pairs arr = arr // valPairs
    where valPairs = fmap (\(i, f) -> (i, f (arr ! i))) pairs
