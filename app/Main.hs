...
keyMap = M.fromList $
    zip [Scancode1, Scancode2, Scancode3, Scancode4, ScancodeQ, ScancodeW, ScancodeE, ScancodeR, ScancodeA, ScancodeS, ScancodeD, ScancodeF, ScancodeZ, ScancodeX, ScancodeC, ScancodeV]
...
loop emu = do
    ...
    let emu' = case pressedKey of
            Nothing -> if emu^.chip8.key `elem` releasedKeys
...
render renderer m = 
    forM_ (assocs (m^.display)) (\((x,y), pix) -> do
